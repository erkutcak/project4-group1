...
<<<<<<< HEAD
        ...
        <p>{item.category}</p>
=======
        {item.image ? <img className='item-image' src={item.image} alt={item.name}/> : <img className='placeholder-image' src={logo} alt={item.name} placeholder="https://www.iconpacks.net/icons/2/free-sale-icon-1994-thumb.png"/>}
        <h2 className="item-name">{item.name}</h2>
        <h3 className='item-price'>$ {item.price}</h3>
        <p className='item-category'>{item.category}</p>
>>>>>>> 8653f5ab08040584d14d4ee8c2ba36dc02101942
